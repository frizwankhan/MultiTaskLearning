...
# I took class labels reference from cityscapes repo https://github.com/mcordts/cityscapesScripts/blob/master/cityscapesscripts/helpers/labels.py
...
class CITYSCAPES3D(data.Dataset):
    def __init__(self, config, root, split=["train"], is_transform=True,
                 img_size=[1024, 2048], augmentations=None, task_list=['semseg', 'depth'], ignore_index=255):
        ...
        self.dd_label_map_size = [s/config.label_map_ratio for s in img_size]

        self.task_flags = {'semseg': True, 'depth': True}
        ...
        for _split in self.split:
            ...
            self.depth_base = os.path.join(self.root, 'disparity')
            self.camera_base = os.path.join(self.root, 'camera') 
        ...
        # self.valid_classes = [7, 8, 11, 12, 13, 17, 19, 20, 21, 22, 23, 24, 25, 26, 31, 32, 33]
